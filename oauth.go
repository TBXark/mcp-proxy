...
func (s *OAuthServer) loadClients() {
	...
	data, err := ioutil.ReadFile(s.persistenceFile)
	if err != nil {
...
func (s *OAuthServer) saveClients() {
	...
	if err := ioutil.WriteFile(s.persistenceFile, data, 0600); err != nil {
...
func (s *OAuthServer) showSuccessPage(w http.ResponseWriter, r *http.Request, redirectURI, code, state, username string) {
	...
	// HTML success page with auto-redirect
	html := `<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="success-container">
        ...
        <div class="redirect-info" id="redirect-text">
            Redirecting to Claude Desktop in <span id="countdown">3</span> seconds...
        </div>
        ...
        <div id="manual-redirect" class="manual-redirect">
            ...
            <a href="` + redirectURL.String() + `">Click here to continue to Claude Desktop</a>
        </div>
    </div>
</body>
</html>`
